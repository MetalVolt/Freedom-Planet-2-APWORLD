...
item_base_id = 9132022000 
#Need to determine this. Gotta be in the Unity code somewhere. EDIT 04/05/2025: For FP2, we just figure out an...
#...unused starting point. The value used above seems to be one; but, can be changed.
...
item_table: Dict[str, FP2ItemData] = {
    ...
    "Mirror Trap": FP2ItemData(ItemClassification.trap, 1, 46, "Traps"),
    "Moon Gravity Trap": FP2ItemData(ItemClassification.trap, 1, 47, "Traps"),
    "Double Gravity Trap": FP2ItemData(ItemClassification.trap, 1, 48, "Traps"),
}   #"Pie Trap": FP2ItemData(ItemClassification.trap, 1, 48, "Traps"), #This would be a good one if possible, would be funny.
    #Vinyls
...
filler_items: List[str] = ["Gold Gem", "Crystals x1000"] #whatever we decide to use as a filler can be put here.
